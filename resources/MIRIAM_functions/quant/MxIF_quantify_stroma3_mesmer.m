function [NoOvlp, Stats]=MxIF_quantify_stroma3_mesmer(i, WatCellSeg, AFRemoved, AFList, PosList, ThreshList, DAPI, tumorMask, OutPos)
...
for j = 1:length(AFList) %quantify each marker
    fprintf([AFList{j} ' '])
    AFim=imread(join([AFRemoved AFList{j} '_AFRemoved_' PosList{i} '.tif'],"")); %read biomarker image
    %AForig=AFim;
    ...
    AFQuantCell.Properties.VariableNames={join(['Median_Cell_' AFList{j}],"")};
    ...
    %             AFQuantMem.Properties.VariableNames={join(['Median_Mem_' AFList{j}],"")};
    ...
    %             AFQuantCyt.Properties.VariableNames={join(['Median_Cyt_' AFList{j}],"")};
...
if~isempty(ThreshList)
    ...
    for t=1:length(ThreshList)
        ...
        thresh_image=imread(join([strrep(AFRemoved, join([filesep, "AFRemoved Tiles"],""), join([filesep, "Thresholded Tiles"],"")), ThreshList{t}, "_", PosList{i}, ".png"],""));
        ThreshQuant=struct2table(regionprops(WatCellSeg, thresh_image, {'PixelValues'}));
        ThreshQuant=table(cellfun(@nansum, ThreshQuant{:,1}));
        ThreshQuant = renamevars(ThreshQuant, ["Var1"], [join(['Area_Thresh_' strrep(ThreshList{t}, "_THRESHOLDED", "")],"")]);
...
NoOvlp = labeloverlay(adapthisteq(DAPI),WatCellSeg);
%CellBorders=imdilate(WatCellSeg>0, ones(3,3)) & ~WatCellSeg>0;

%create NoOvlpImages
%NoOvlp(:,:,1)=DAPI;
%NoOvlp(:,:,2)=CellBorders;
%NoOvlp(:,:,3)=DAPI;
