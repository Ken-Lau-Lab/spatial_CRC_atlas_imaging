...
%set up cells
        ...
        if ~isempty(annotate)
            ...
            none=annotate(:,:,3);
            top=xor(top, none);
            bottom=xor(bottom, none);
            
            annotateTop=struct2table(regionprops(WatCellSeg, top, { 'PixelValues' }));
            annotateTop=table(cellfun(@nanmax, annotateTop{:,1}));
            annotateTop.Properties.VariableNames={['Top']};
            
            
            annotateBottom=struct2table(regionprops(WatCellSeg, bottom, { 'PixelValues' }));
            annotateBottom=table(cellfun(@nanmax, annotateBottom{:,1}));
            annotateBottom.Properties.VariableNames={['Bottom']};
            
            Stats=[Stats annotateTop annotateBottom];
