function resegmuc2Nuc_quant_cell_cytell_ML(SlideDir, NucList, MemList, EpiList ,quantify, start)
...
for i=start:length(PosList)
    ...
    if ~exist([OutDir{15} OutPos{i} '_stack.tif'])
        ...
        for j=1:(length(AFList))
            ...
            imwrite(imread([SlideDir '/AFRemoved/' AFList{j} '_AFRemoved_pyr16_spot_'  OutPos{i} '.tif']), [OutDir{15} OutPos{i} '_stack.tif'], 'writemode', 'append', 'Compression', 'lzw');
