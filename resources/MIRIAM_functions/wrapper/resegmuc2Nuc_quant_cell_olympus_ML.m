function resegmuc2Nuc_quant_cell_olympus_ML(SlideDir, NucList, MemList, EpiList , StromaList, quantify, start)
...
%for i= 3:length(PosList)
    ...
    %check to see if files exist; no: generate files ; yes: read saved images
    if ~exist([OutDir{14} 'ML_' OutPos{i} '.jpg'])
        fprintf('ML PreProcessing; ')
        ...
        str=strcat(AFRemoved, StromaList ,['_AFRemoved_' PosList{i} '.tif']); %get path and filenames for epithelial markers
        %generate epithelial mask
        R_ML=ML_processing2(epi,.995);
        G_ML=ML_processing2(str,.98);  %adapthisteq(imread(epi{1}, 'ClipLimit', 0.2));
        B_ML=mat2gray(imadjust(imread(DapiList{i}),stretchlim(imread(DapiList{i}),[0 .999])));
        %epiMask=tubsegbatch(epi, pixadj*10, pixadj*0.00001, pixadj*0.01);
        ML=(cat(3, R_ML, G_ML, B_ML));
        ML=imresize(ML,[1024 1024]);
        imwrite(ML, [OutDir{14} 'ML_' OutPos{i} '.jpg'] )
        
    end   
        
        %write image generated from tubesegbatch- forcing into 8 bit binary
        %(0 or 255)
        %imwrite(uint8(255*(epiMask>0)), [OutDir{3} 'EpiMask_' OutPos{i} '.png'] ) %write epithelial mask borders
    if~ exist([OutDir{3} 'EpiMask_' OutPos{i} '.png'])
        fprintf('EpiMask Processing; ')
            if ~exist([OutDir{14} 'Prob_ML_' OutPos{i} '.jpg'])
                
                fprintf('No Probability File\n')
                continue
            end
            epiMask=imread([OutDir{14} 'Prob_ML_' OutPos{i} '.jpg']);
    ...
    %if the epithelial mask is blank, skip further processing
    if max(epiMask(:))==0 
            fprintf('Empty Epithelial Mask')
            fprintf('\n')
            continue
    end
