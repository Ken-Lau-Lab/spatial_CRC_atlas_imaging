function CellSegQuant_CellDive_COLON_MAP_Stroma_Watershed_Threshold(SlideDir, quantify, shape, stroma, tumor, threshold, annotate, start)
...
AFList=erase(AFList, "DAPI");
AFList=AFList(~cellfun('isempty',AFList));
...
if threshold ==1
    ...
    ThreshList=imageDatastore(join(["/Users/etmckinley/Dropbox (VUMC)/COLON MAP/Adenomas/THRESHOLDED/" Slide "/*threshold.png"], "")).Files;
...
parfor i=start:length(PosList)
    ...
    if quantify ==1
        if ~exist([OutDir{14} 'mem_' OutPos{i} '_stack_Probabilities.png'], 'file') && stroma == 0
        ...
        if exist([OutDir{14} 'epi_' OutPos{i} '_stack_Probabilities.png'], 'file')
            ...
            FileInfo=dir([OutDir{13} 'PosStats_' OutPos{i} '.csv']);
            date=datestr(FileInfo.date,29);
            
            FileInfoS=dir([OutDir{13} 'StrPosStats2_' OutPos{i} '.csv']);
            dateS=datestr(FileInfoS.date,29);
            if date >= datetime("2021-04-07") && dateS >= datetime("2021-04-07")
                continue
            end
            
            if ~(date >= datetime("2021-04-07"))
    ...
    if ~(dateS >= datetime("2021-04-07"))
        if stroma==1
            
            %if ~exist([OutDir{13} 'StrPosStats2_' OutPos{i} '.csv'], 'file')
            fprintf('\nStromal Quant; ')
            epiMask=imread([OutDir{14} 'epi_' OutPos{i} '_stack_Probabilities.png']);
            if mean(mean(epiMask(:,:,1)))<2 || mean(mean(epiMask(:,:,2)))== 0
                fprintf('Blank Position\n')
            ...
            fastPOS = FastPeakFind(NUCimg, 1, filt2, 2, 1);
            ...
            if max(max(water)) >9
                [strStats]=MxIF_quantify_stroma2_thresh(i, water, AFRemoved, AFList, PosList, ThreshList, NUCimg,  tumorMask, OutPos);
                if ~isempty(strStats)
                    
                    writetable(strStats, [OutDir{13} 'StrPosStats2_' OutPos{i} '.csv'], 'WriteVariableNames', true);
