function resegmuc2Nuc_quant_cell_cytell_ML_IL_str(SlideDir, quantify, stroma, start)
...
parfor i=start:length(PosList)
    ...
    if ~exist([OutDir{15} OutPos{i} '_stack.tif'])
        ...
        imwrite(imread(DapiList{i}), [OutDir{15} OutPos{i} '_stack_all.tif'], 'Compression', 'lzw');
        %[s1 s2]=size(DapiImg);
    ...
    if stroma==1
        fprintf('Stromal Quant; ')
        if ~exist([OutDir{14} 'str_' OutPos{i} '_stack_Probabilities.png'])
            fprintf('No Epithelial Probability File\n')
            continue
        end
        if exist([OutDir{13} 'StrPosStats_' OutPos{i} '.csv']) && exist([OutDir{4} 'StrNovlp_' OutPos{i} '.png'])
            ...
            %stromal_label=bwlabel(stromal_nuclei,4);
            stromal_grow=imdilate(stromal_nuclei, strel('disk',3));
            stromal_label= watershed(imimposemin(uint8(stromal_grow),stromal_nuclei),4);
