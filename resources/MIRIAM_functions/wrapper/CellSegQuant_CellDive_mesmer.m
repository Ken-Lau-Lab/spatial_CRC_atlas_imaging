function CellSegQuant_CellDive_mesmer(SlideDir, quantify, shape, stroma, tumor, start, ilastik_epi_file, ilastik_mem_file, ilastik_app_loc, stack_markers)
...
AFList=AFList.Files;
AFList = AFList(~contains(AFList, "downsample"));
%AFList=AFList{1,1};
%PosList=AFList;
...
PosList=AFList(:,2);
AFList=split(AFList(:,1), join([filesep 'AFRemoved Tiles' filesep],""));
AFList=AFList(:,2);
...
ThreshList=[];
if exist(join([SlideDir, filesep, "Thresholded Tiles", filesep], ""), 'dir')
    ThreshList=imageDatastore(join([SlideDir, filesep, "Thresholded Tiles", filesep, "*THRESHOLDED*"], "")).Files;
    ThreshList = strrep(ThreshList, join([SlideDir, filesep, "Thresholded Tiles", filesep],""),"");
...
fprintf(join(['Segmentation of: ' SlideDir ' ; ' num2str(length(PosList)) ' Positions;\n' ],""))
...
parfor i=  start: length(PosList)


    fprintf(join([ SlideDir ' ' OutPos{i} ': '],""))
    tic
    %make Stacks of AFRemoved images and Dapi if they don't exist
    if ~exist(join([OutDir{15} OutPos{i} '_stack.tif'],""), 'file')
        seg_markers = stack_markers;
        fprintf(['Stack: ' OutPos{i} '\n'])
        imwrite(imread(DapiList{i}), join([OutDir{15} OutPos{i} '_stack.tif'],""), 'Compression', 'lzw'); %write out DAPI
        for j=1:(length(seg_markers)) %loop through the AFRemoved images and append to tiff stack
           
            imwrite(imread(join([SlideDir filesep 'AFRemoved Tiles' filesep seg_markers{j} '_AFRemoved_' OutPos{i} '.tif'], "")), join([OutDir{15} OutPos{i} '_stack.tif'],""), 'writemode', 'append', 'Compression', 'lzw');
...
%     if ~exist(join([OutDir{14} 'epi_' OutPos{i} '_stack_Probabilities.png'],""), 'file')
%         proj_path = ilastik_epi_file;
%         if ~exist(proj_path, 'file')
%             continue
%         end
%         file_info= dir(join([OutDir{15} OutPos{i} '_stack.tif'],""));
...
% 
%         if ispc
%             cd_path = strrep(ilastik_app_loc, "\ilastik.exe","");
%             command = join(['cd "' cd_path '"'],"");
%             command1 = join(['.\ilastik.exe'  ' --headless --project="' proj_path '" --output_format=png --output_filename_format="' outPath '" "' stackPath '"'],"");
%             final_command = join([command, command1], "&");
%             [ ~, ~ ] = system(final_command);
%         end
% 
%        if ismac
%            command1 = join([ilastik_app_loc ' --headless --project="' proj_path '" --output_format=png --output_filename_format="' outPath '"  "' stackPath '"'],"");
%            [ ~, ~ ] =  system(command1);
%        end
% 
% 
...
%     if ~exist(join([OutDir{14} 'mem_' OutPos{i} '_stack_Probabilities.png'],""), 'file')
...
for i=  start: length(PosList)
    ...
    if~ exist(join([OutDir{3} 'EpiMask_' OutPos{i} '.png'],""), 'file')
        
        if ~exist(join([OutDir{14} 'epi_' OutPos{i} '_stack_Probabilities.png'],""), 'file')
            continue
        end
        fprintf('EpiMask Processing; ')
        epiMask=imread(join([OutDir{14} 'epi_' OutPos{i} '_stack_Probabilities.png'],""));
        ...
        epiMask=logical(imread(join([OutDir{3} 'EpiMask_' OutPos{i} '.png'],"")));
    ...
    if exist(join([OutDir{14} 'epi_' OutPos{i} '_stack_Probabilities.png'],""), 'file')
        ...
        if ~exist(join([OutDir{5} 'NucMask_' OutPos{i} '.png'],""), 'file') || ~exist(join([OutDir{7} 'SuperMem_' OutPos{i} '.tif'],""), 'file') || ~exist(join([OutDir{10} 'MemMask_' OutPos{i} '.png'],""), 'file')
            %Read in probability image for membrane and nucleus
            if ~exist(join([OutDir{14} 'mem_' OutPos{i} '_stack_Probabilities.png'],""), 'file')
                continue
            end
            Probs=imread(join([OutDir{14} 'mem_' OutPos{i} '_stack_Probabilities.png'],""));


            mask=uint8(255*(Probs(:,:,2)>255*.6)); %set nuclear probability >0.6 as nuclear mask
            imwrite(mask, [OutDir{5} 'NucMask_' OutPos{i} '.png'] )
            imwrite(Probs(:,:,1), join([OutDir{7} 'SuperMem_' OutPos{i} '.tif'],"") ) %write 16 bit tiff
            imwrite(uint8(255*(Probs(:,:,1)>255*.6)), join([OutDir{10} 'MemMask_' OutPos{i} '.png'],"") ) %write 16 bit tiff
            MemMask=im2bw(imread([OutDir{10} 'MemMask_' OutPos{i} '.png']));

        else
            %read files if previously generated
            mask=imread( join([OutDir{5} 'NucMask_' OutPos{i} '.png'],""));
            SuperMem=imread( join([OutDir{7} 'SuperMem_' OutPos{i} '.tif'],""));
            MemMask=im2bw(imread(join([OutDir{10} 'MemMask_' OutPos{i} '.png'],"")));
        end
        ...
        %check to see if files exist; no: generate files ; yes: read saved images
        if ~exist(join([OutDir{12} 'CellSegFinal_' OutPos{i} '.tif'],""), 'file') || ~exist(join([OutDir{11} 'NucMaskFinal_' OutPos{i} '.png'],""), 'file')

            fprintf('CellSeg; ')

            %check to see if files exist; no: generate files ; yes: read saved images
            if ~exist(join([OutDir{1} 'L2_' OutPos{i} '.tif'],""), 'file')
                ...
                L2=L2.*epiMask;
                imwrite(uint16(L2), join([OutDir{1} 'L2_' OutPos{i} '.tif'],"") ) %write 16 bit tiff
            ...
            %check to see if files exist; no: generate files ; yes: read saved images
            if ~exist(join([OutDir{1} 'CellSeg_' OutPos{i} '.tif'],""), 'file')
                MemMask=im2bw(imread(join([OutDir{10} 'MemMask_' OutPos{i} '.png'],"")));
                CellSeg= ReSegCells(L2, MemMask);
                imwrite(uint16(CellSeg), join([OutDir{1} 'CellSeg_' OutPos{i} '.tif'],"") )
            else
                CellSeg=imread( join([OutDir{1} 'CellSeg_' OutPos{i} '.tif'],""));
            end
            %   toc

            %check to see if files exist; no: generate files ; yes: read saved images
            if ~exist(join([OutDir{12} 'CellSegFinal_' OutPos{i} '.tif'],""), 'file')
                CellSeg=imread( join([OutDir{1} 'CellSeg_' OutPos{i} '.tif'],""));
                SuperMem=imread( join([OutDir{7} 'SuperMem_' OutPos{i} '.tif'],""));
                Probs=imread(join([OutDir{14} 'mem_' OutPos{i} '_stack_Probabilities.png'],""));
                ...
                %write out data
                imwrite(uint16(WatCellSeg), join([OutDir{12} 'CellSegFinal_' OutPos{i} '.tif'],"") )
                imwrite(uint8(255*(mask>0)), join([OutDir{11} 'NucMaskFinal_' OutPos{i} '.png'],"") )
            else
                %read in data
                WatCellSeg=imread( join([OutDir{12} 'CellSegFinal_' OutPos{i} '.tif'],""));
                mask=logical(imread(join([OutDir{11} 'NucMaskFinal_' OutPos{i} '.png'],"")));
            end
        else
            WatCellSeg=imread( join([OutDir{12} 'CellSegFinal_' OutPos{i} '.tif'],""));
            mask=logical(imread(join([OutDir{11} 'NucMaskFinal_' OutPos{i} '.png'],"")));
        ...
        if quantify ==1
            ...
            if exist(join([OutDir{13} 'PosStats_' OutPos{i} '.csv'],""), 'file') && exist(join([OutDir{4} 'Novlp_' OutPos{i} '.png'],""), 'file')
                ...
                if tumor==0
                    ...
                    [Stats, NoOvlp]=MxIF_quantify_threshold(i, WatCellSeg, AFRemoved, AFList, PosList, ThreshList, mask, MemMask, [], OutPos);
                end
                if tumor==1
                    if length(unique(WatCellSeg)) <20
                        continue
                    ...
                    %    continue
                    %end
                    if  ~exist(join([OutDir{16} 'TumorMask_' OutPos{i} '.png'],""), 'file')
                        %[tumorMask,~ , ~]=imread([OutDir{14} 'tum_' OutPos{i} '_stack_Probabilities.png']);
                        tumorMask = false(size(mask,1), size(mask,2));
                    else
                        tumorMask= ( imread(join([OutDir{16} 'TumorMask_' OutPos{i} '.png'],"")));
                    ...
                    [Stats, NoOvlp]=MxIF_quantify_threshold(i, WatCellSeg, AFRemoved, AFList, PosList, ThreshList, mask, MemMask, tumorMask, OutPos);
                ...
                writetable(Stats, join([OutDir{13} 'PosStats_' OutPos{i} '.csv'],""), 'WriteVariableNames', true);
                %dlmwrite([OutDir{13} 'PosStats_' OutPos{i} '.csv'], (Stats), '-append');
                imwrite(double(NoOvlp), join([OutDir{4} 'Novlp_' OutPos{i} '.png'],"") )
            end
    ...
    if stroma==1 && quantify ==1
        ...
        if  ~exist(join([OutDir{13} 'StrPosStats_' OutPos{i} '.csv'],""), 'file')
            fprintf(join(['\n' OutPos{i} ': Stromal Quant; ']),"")
            ...
            if exist(join([OutDir{12} 'StrCellSegFinal_' OutPos{i} '.tif'],""), 'file')
                epiMask=logical(imread(join([OutDir{3} 'EpiMask_' OutPos{i} '.png'],"")));
                water = imread(join([OutDir{12} 'StrCellSegFinal_' OutPos{i} '.tif'],""));
                water(epiMask==1)=0;
                ...
                fprintf('No Mesmer Segmentation\n')
                continue
            end
            
            
            %exist(join([OutDir{13} 'StrPosStats_' OutPos{i} '.csv'],""), 'file')

            

            %imwrite(uint16(water), join([OutDir{12} 'StrCellSegFinal_' OutPos{i} '.tif'],"") )
            ...
            if tumor==1
                ...
                [strNoOvlp, strStats]=MxIF_quantify_stroma3_mesmer(i, water, AFRemoved, AFList, PosList,ThreshList, NUCimg,  tumorMask, OutPos);
            ...
            if length(unique(water(:))) >9
                [strNoOvlp, strStats]=MxIF_quantify_stroma3_mesmer(i, water, AFRemoved, AFList, PosList,ThreshList, NUCimg,  tumorMask, OutPos);
                strStats =rmmissing(strStats,'DataVariables',{'Cell_Centroid_X'});

            end
            if ~isempty(strStats)

                writetable(strStats, join([OutDir{13} 'StrPosStats_' OutPos{i} '.csv'],""), 'WriteVariableNames', true);
                ...
                imwrite(strNoOvlp, join([OutDir{4} 'StrNovlp_' OutPos{i} '.png'],"") )
            end
            %end
    ...
    if shape==1
        ...
        %save to .mat file
        if exist(join([OutDir{12} 'CellSegFinal_' OutPos{i} '.tif'],""), 'file') && ~exist(join([OutDir{17} 'CellShape_' OutPos{i} '.mat' ],""), 'file')
            fprintf('\nCell Shape Pre-Processing; ')
            CellImages=imread(join([OutDir{12} 'CellSegFinal_' OutPos{i} '.tif'],""));
        ...
        if i == length(PosList)
            if ~exist(join([OutDir{17} 'autoencoder.mat'],""), 'file') && ~exist(join([OutDir{17} 'encoded_cells.csv'],""), 'file')
                fprintf('\nTraining Autoencoder; ')
                FileList= dir(join([OutDir{17} 'CellShape*.mat'],""));
                ...
                save(join([OutDir{17} 'autoencoder.mat'],""), 'autoencoder')
                writetable(trainList, join([OutDir{17} 'encoded_cells.csv'],""));
