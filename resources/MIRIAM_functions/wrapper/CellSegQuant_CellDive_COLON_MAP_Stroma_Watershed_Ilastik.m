function CellSegQuant_CellDive_COLON_MAP_Stroma_Watershed_Ilastik(SlideDir, quantify, shape, stroma, tumor, annotate, start)
...
%for i=  200: length(PosList)
    ...
    if ~exist([OutDir{14} 'epi_' OutPos{i} '_stack_Probabilities.png'], 'file')
        proj_path = '/Volumes/CellDive/cody_batch2_epi.ilp';
        if ~exist(proj_path, 'file')
            continue
        end
        file_info= dir([OutDir{15} OutPos{i} '_stack.tif']);
        if file_info.bytes < 2000000
        ...
        command1 = ['/Applications/ilastik-1.4.0b11-OSX.app/Contents/ilastik-release/run_ilastik.sh --headless --project=' proj_path ' --output_format=png --output_filename_format=' outPath ' ' [OutDir{15} OutPos{i} '_stack.tif']];
    ...
    if ~exist([OutDir{14} 'mem_' OutPos{i} '_stack_Probabilities.png'], 'file')
        proj_path = '/Volumes/CellDive/cody_batch2_mem.ilp';
        if ~exist(proj_path, 'file')
            continue
        end
    ...
    if stroma==1
        ...
        if tumor == 0
         ...
         %quantify markers in cells and write out data
            if max(max(water)) >9
                strStats=MxIF_quantify_stroma2_thresh(i, water, AFRemoved, AFList, PosList, [], NUCimg,  tumorMask, OutPos);
